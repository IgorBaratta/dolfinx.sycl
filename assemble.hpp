...
namespace dolfinx_sycl::assemble
{
    ...
    device_data_t
    send_data_to_device(cl::sycl::queue &queue, const dolfinx::fem::Form<double> &L,
                        const dolfinx::fem::Form<double> &a)
    {
        auto mesh = L.mesh();
        auto dofmap = L.function_spaces()[0]->dofmap();
        int tdim = mesh->topology().dim();
        std::int32_t ndofs = dofmap->index_map->size_local() + dofmap->index_map->num_ghosts();
        std::int32_t ncells = mesh->topology().index_map(tdim)->size_local() + mesh->topology().index_map(tdim)->num_ghosts();
    ...
    double *assemble_vector(cl::sycl::queue &queue, device_data_t &data)
    {
        ...
        assemble_vector_ext(queue, b, data.x, data.xdofs, data.coeffs, data.ncells, data.ndofs, data.ndofs_cell);
